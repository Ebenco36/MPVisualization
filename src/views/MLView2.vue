        ...
        <div class="center-div card mb-4">
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                            ...
                            <form>
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="inputText"
                                        placeholder="Type something..." v-model="pdb_codes">
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-primary" style="color:white!important;" id="saveButton"
                                @click="downloadSampleFile">{{ load_status ? "Loading ..." : "Download Sample!"
                                }}</button>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in accuracy_data" :key="index" :class="{
            ...
            <div class="card-body" style="text-align: justify;">
                ...
                <form @submit.prevent="uploadFile" class="row">
                    ...
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-primary btn-large mt-3" style="color: white;"> {{
                            load_status ? "Loading ..." : "Predict Now!" }}</button>
                    </div>
                </form>
                ...
                <div>
                    ...
                    <a class="badge badge-info text-white p-2 cursor-pointer" v-if="!isEmpty(accuracy_data)"
                        data-toggle="modal" data-target="#exampleModalCenter">View metrics based on {{
                            dim_reduction.toUpperCase() }}</a>
                </div>
                <br />
                    ...
                    <div class="row g-3">
                        <div class="col-md-6 d-flex">
                            <div class="w-100 d-flex flex-column">
                                <GraphView id="pca_chart" :summary="dimensionality_reduction_chart?.pca_chart"
                                    label="Principal Component Analysis (PCA)" />
                            </div>
                        </div>
                        <div class="col-md-6 d-flex">
                            <div class="w-100 d-flex flex-column">
                                <GraphView id="t-SNE_chart" :summary="dimensionality_reduction_chart?.t_sne_chart"
                                    label="t-Distributed Stochastic Neighbor Embedding (t-SNE)" />
                            </div>
                        </div>
                        <div class="col-md-6 d-flex">
                            <div class="w-100 d-flex flex-column">
                                <GraphView id="umap_chart" :summary="dimensionality_reduction_chart?.umap_chart"
...
const sample_data = ref([
    [
        "pdb_code", "topology_subunit", "thickness",
        "subunit_segments", "tilt", "gibbs", "membrane_topology_in", "membrane_topology_out"
    ],
    ["A123", 3.0, 0, 4.0, 15, -5.5, "cytoplasmic side", "periplasm"],
    ["B123", 25.8, 10, 12, 5, -26.4, "cytoplasmic side", "lumenal side"],
    ["C123", 2.5, 5, 3.0, 15, -5.5, "cytoplasmic side", "periplasm"],
    ["D123", 25.8, 10, 12, 3, -26.4, "cytoplasmic side", "lumenal side"],
    ["E123", 2.5, 0, 3.0, 15, -5.5, "cytoplasmic side", "periplasm"],
    ["F123", 25.8, 1, 12, 9, -26.4, "cytoplasmic side", "lumenal side"]
])
...
const uploadFile = async () => {
    ...
    reader.onload = async (e) => {
        ...
        const headerOffset = rows.length > 0 && rows[0].includes(',') ? 1 : 0;
        const dataRows = rows.length - headerOffset;

        if (dataRows > 5) {
            error.value = "CSV file must contain a maximum of 5 data rows";
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                link.href = url;
                link.setAttribute('download', `MetaMP_Predictions_${model_.value}_${dim_reduction.value}.csv`);
                document.body.appendChild(link);
                link.click();
                window.URL.revokeObjectURL(url);
                link.remove();
